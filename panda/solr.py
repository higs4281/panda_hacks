...
def query(core, q, limit=10, offset=0, sort='_docid_ asc'):
    ...
    response = requests.get(url, params={ 'q': q, 'mm': '100%', 'start': offset, 'rows': limit, 'sort': sort }, headers={ 'Content-Type': 'application/json' })
...
def query_grouped(core, q, group_field, limit=10, offset=0, sort='_docid_ asc', group_limit=settings.PANDA_DEFAULT_SEARCH_ROWS_PER_GROUP, group_offset=0):
    ...
    """
    url = ''.join([settings.SOLR_ENDPOINT, '/', core, '/select'])
    response = requests.get(url, params={ 'q': q, 'mm': '100%', 'start': offset, 'rows': limit, 'sort': sort, 'group': 'true', 'group.field': group_field, 'group.limit': group_limit, 'group.offset': group_offset, 'group.ngroups': 'true' }, headers={ 'Content-Type': 'application/json' })
